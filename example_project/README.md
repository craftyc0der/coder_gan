...
| `install_order(graph) -> Result<Vec<String>, ResolverError>` | Leaves-first ordering | Same dependency-first order as `topological_sort` |

Because an edge `A -> B` means "A depends on B", a valid topological order for
this project already places dependencies before dependents. That means
`install_order` and `topological_sort` should both return leaves-first /
dependency-first order for the same graph.
...
**Key rule:** All tests live in a `tests/` subfolder next to the business logic,
...
- [ ] **Team Graph**: Write tests in `src/graph/tests/mod.rs` (>=10 tests)
- [ ] **Team Resolver**: Write tests in `src/resolver/tests/mod.rs` (>=10 tests)
